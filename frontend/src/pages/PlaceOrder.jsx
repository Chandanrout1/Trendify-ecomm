          ...
          <div className='mt-12'>
            ...
            <div className='flex flex-col lg:flex-row gap-3'>
              <div  onClick={()=>setMethod('stripe')} className='flex items-center gap-3 border px-3 p-2 cursor-pointer'>
                <p className={`min-w-3.5 h-3.5 border rounded-md ${method === 'stripe' ? 'bg-green-400' : ''}`}></p>
                <img className='h-5 mx-4' src={assets.stripe_logo} alt="" />
              </div>
              <div onClick={()=>setMethod('razorpay')} className='flex items-center gap-3 border px-3 p-2 cursor-pointer'>
                <p className={`min-w-3.5 h-3.5 border rounded-md ${method === 'razorpay' ? 'bg-green-400' : ''}`}></p>
                <img className='h-5 mx-4' src={assets.razorpay_logo} alt="" />
              </div>
              <div onClick={()=>setMethod('cod')} className='flex items-center gap-3 border px-3 p-2 cursor-pointer'>
                <p className={`min-w-3.5 h-3.5 border rounded-md ${method === 'cod' ? 'bg-green-400' : ''}`}></p>
                <p className='text-gray-700 text-sm font-medium mx-4'>Cash on Delivery</p>
              </div>
            </div>
            <div className='w-full text-end mt-8'>
              <button onClick={()=> navigate("/orders")} className='bg-blue-600 text-white px-16 py-3 text-sm rounded-md'>Place Order</button>
            </div>
          </div>
